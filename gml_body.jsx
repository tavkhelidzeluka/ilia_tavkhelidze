...
// --- Whole body (no cut) ---------------------------------------------------
function buildWholeGeometry(m, n, R, r, segs) {
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    for (let k = 0; k < m; k++) {
      ...
      const [px, py, pz] = localToWorld(theta, bU, bV, R, m, n);
...
function buildWedgeGeometry(m, n, R, r, segs, K, phiStart, phiEnd, sep, palette, thetaMax = 2 * Math.PI, endCaps = false) {
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    for (let j = 0; j < K; j++) {
      ...
      const [px, py, pz] = localToWorld(theta, bU + dispU, bV + dispV, R, m, n);
      positions.push(px, py, pz);
      colors.push(...SURF);
    }
  }
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    for (let q = 0; q < NCUT; q++) {
      ...
      const [px, py, pz] = localToWorld(theta, uL, vL, R, m, n);
      positions.push(px, py, pz);
      colors.push(...CUT);
    }
  }
  ...
  if (endCaps) {
    ...
    addEndCap(positions, indices, colors, wedgePoly, 0, dispU, dispV, R, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, wedgePoly, thetaMax, dispU, dispV, R, m, n, palette.cut, false);
  }
...
function buildRidgeLines(m, n, R, r, segs) {
  const lines = [];
  for (let k = 0; k < m; k++) {
    const pos = [];
    const phi = (2 * Math.PI * k) / m;
...
function buildStripGeometry(m, n, R, r, segs, phiCut, dLeft, dRight, sep, palette, thetaMax = 2 * Math.PI, endCaps = false) {
  ...
  if (endCaps) {
    addEndCap(positions, indices, colors, stripPoly, 0, dispU, dispV, R, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, stripPoly, thetaMax, dispU, dispV, R, m, n, palette.cut, false);
  }
...
function addEndCap(positions, indices, colors, polygon2D, theta, dispU, dispV, R, m, n, color, reversed) {
  const baseIdx = positions.length / 3;
  const verts = reversed ? polygon2D.slice().reverse() : polygon2D;
  for (const [u, v] of verts) {
    const [px, py, pz] = localToWorld(theta, u + dispU, v + dispV, R, m, n);
    positions.push(px, py, pz);
...
function buildRegionGeometry(m, n, R, r, segs, regionPoly, chords, sep, palette, thetaMax = 2 * Math.PI, endCaps = false, cutTags = null) {
  ...
  if (endCaps) {
    addEndCap(positions, indices, colors, regionPoly, 0, dispU, dispV, R, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, regionPoly, thetaMax, dispU, dispV, R, m, n, palette.cut, false);
  }
  const g = new THREE.BufferGeometry();
  g.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
  g.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));
      ...
      rebuild(mV, nV, ridgesOn, cutOn, mode, slices, dParam, cutPhiRad, sep, thetaMax, endCaps, p1Rad, p2Rad, blade, bladeAmt, profile, gradientOn, soundMode) {
        ...
        if (cutOn && mode === 'p2p') {
          ...
          if (chords.length > 0) {
            ...
            for (const [pat, regionPoly] of regions) {
              ...
              const g = buildRegionGeometry(mV, nV, R, r, segs, defPoly, chords, sep, palette, thetaMax, endCaps, cutTags);
            ...
            const g = buildWholeGeometry(mV, nV, R, r, segs);
          ...
          for (let j = 0; j < numStrips; j++) {
            ...
            if (stripPoly.length >= 3) {
              ...
              const g = buildRegionGeometry(mV, nV, R, r, segs, defPoly, parallelChords, sep, palette, thetaMax, endCaps, cutTags);
              if (g && gradientOn) applyGradient(g);
              if (g) {
                const mesh = new THREE.Mesh(g, matForOrbit(oIdx));
                mesh.userData.orbitIdx = oIdx;
          ...
          for (let w = 0; w < numWedges; w++) {
            ...
            const oIdx = w % orbits;
            const palette = PIECE_PALETTE[oIdx % PIECE_PALETTE.length];
            const g = buildWedgeGeometry(mV, nV, R, r, segs, K, phiStart, phiEnd, sep, palette, thetaMax, endCaps);
          ...
          s.objects.push(mesh);
          if (ridgesOn) {
            for (const pos of buildRidgeLines(mV, nV, R, r, segs)) {
        ...
        if (soundMode) {
          ...
          for (const obj of s.objects) {
            ...
            // Recover (theta, phi) from each vertex's xyz. theta is just the
            // polar angle around the torus axis. phi is the cross-section
            // angle in the rotating frame, recovered by untwisting by α.
            const positions = obj.geometry.attributes.position.array;
            const N = positions.length / 3;
            const wc = new Float32Array(N * 2);
            for (let i = 0; i < N; i++) {
              const x = positions[i * 3];
              const y = positions[i * 3 + 1];
              const z = positions[i * 3 + 2];
              const theta = Math.atan2(y, x);
              const aN = Math.hypot(x, y) - R;
              const aB = z;
              const alpha = (nV / mV) * theta;
              const cA = Math.cos(alpha), sA = Math.sin(alpha);
              const uL =  aN * cA + aB * sA;
              const vL = -aN * sA + aB * cA;
              wc[i * 2] = theta;
              wc[i * 2 + 1] = Math.atan2(vL, uL);
            }
            obj.userData.waveCoords = wc;
