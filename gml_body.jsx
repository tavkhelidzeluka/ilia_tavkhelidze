...
function buildWedgeGeometry(m, n, pathFrame, r, segs, K, phiStart, phiEnd, sep, palette, thetaMax = 2 * Math.PI, endCaps = false) {
  ...
  if (endCaps) {
    ...
    addEndCap(positions, indices, colors, wedgePoly, 0, dispU, dispV, pathFrame, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, wedgePoly, thetaMax, dispU, dispV, pathFrame, m, n, palette.cut, false);
  }
...
function buildStripGeometry(m, n, pathFrame, r, segs, phiCut, dLeft, dRight, sep, palette, thetaMax = 2 * Math.PI, endCaps = false) {
  ...
  if (endCaps) {
    addEndCap(positions, indices, colors, stripPoly, 0, dispU, dispV, pathFrame, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, stripPoly, thetaMax, dispU, dispV, pathFrame, m, n, palette.cut, false);
  }
...
function addEndCap(positions, indices, colors, polygon2D, theta, dispU, dispV, pathFrame, m, n, color, reversed) {
  const baseIdx = positions.length / 3;
  const verts = reversed ? polygon2D.slice().reverse() : polygon2D;
  for (const [u, v] of verts) {
    const [px, py, pz] = localToWorld(pathFrame, theta, u + dispU, v + dispV, m, n);
    positions.push(px, py, pz);
...
function buildRegionGeometry(m, n, pathFrame, r, segs, regionPoly, chords, sep, palette, thetaMax = 2 * Math.PI, endCaps = false, cutTags = null) {
  ...
  if (endCaps) {
    addEndCap(positions, indices, colors, regionPoly, 0, dispU, dispV, pathFrame, m, n, palette.cut, true);
    addEndCap(positions, indices, colors, regionPoly, thetaMax, dispU, dispV, pathFrame, m, n, palette.cut, false);
  }
  const g = new THREE.BufferGeometry();
  g.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
  g.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));
    ...
    stateRef.current = {
      ...
      rebuild(mV, nV, ridgesOn, cutOn, mode, slices, dParam, cutPhiRad, sep, thetaMax, endCaps, p1Rad, p2Rad, blade, bladeAmt, profile, gradientOn, soundMode) {
        ...
        if (soundMode) {
          ...
          s.orbitMats = [];
          // Sound-mode inverse mapping currently assumes a circle path of
          // radius R = 2.3 (matches defaultCirclePathFrame). Path variants
          // will replace this in a later task.
          const R = 2.3;
          for (const obj of s.objects) {
